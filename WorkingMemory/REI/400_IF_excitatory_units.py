...
def plot(v_t_fire, intT, times, N):
    ...
    plt.savefig("memoryless_raster_400.png")
    ...
    plt.savefig("memoryless_fraction_400.png")
...
for i in range(N):
    ...
    for j in range(N):
        if randint(10) % 10 == 1:
            tmp[0, j] = np.random.normal(1, 0.25, 1)
...
U = np.zeros(N)
for i in range(N):
    U[i] = np.clip(np.random.normal(loc = 0.5, scale = 0.25 * 0.25, size = 1), 0.1, 0.9)
...
t_rec = np.zeros(N)
for i in range(N):
    t_rec[i] = np.clip(np.random.normal(loc = 800, scale = 400 * 0.4, size = 1), 5, None)
...
plt.savefig("memory_less_raster_400.png")
...
plt.legend(loc="lower right")
plt.savefig("memory_less_fraction_400.png")
